...
test("总览界面固定展示八种信用点、前五项、中文名与物品图标", () => {
  ...
  assert.match(source, /suppressGuildExchangeAdvisor\(modalData\.modal\)/);
  assert.match(source, /兑换最优推荐/);
  assert.match(source, /请选择兑换物品以计算卖出后替代方案/);
  assert.match(source, /#mwi-guild-exchange-advisor \[hidden\]\{display:none!important\}/);
  assert.match(source, /data-mode="recommendation"\] \.mwi-advisor-option\{display:flex;flex-direction:column;justify-content:center\}/);
  assert.match(source, /core\.rankConversions\(conversions, books, 1\)/);
  ...
  assert.match(source, /state\.exchangeAdvisorModal/);
  ...
  assert.match(source, /exchangeAdvisorTimer/);
  assert.match(source, /exchangeAdvisorVisibilityTimer/);
  assert.match(source, /watchGuildExchangeAdvisorVisibility/);
  assert.match(source, /\}, 250\);/);
  assert.match(source, /scheduleGuildExchangeAdvisor\(80\)/);
  assert.match(source, /document\.addEventListener\("transitionend"/);
  assert.doesNotMatch(source, /window\.setInterval\(refreshGuildExchangeAdvisor, 5000\)/);
  assert.doesNotMatch(source, /卖出税 2%/);
  assert.match(source, /const advisorRect = advisor\.getBoundingClientRect\(\)/);
  ...
  assert.match(source, /data-role="best-option"/);
