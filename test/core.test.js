...
test("总览界面固定展示八种信用点、前五项、中文名与物品图标", () => {
  ...
  assert.match(source, /<th>信用点<\/th><th>兑换<\/th><th>金币价值<\/th>/);
  assert.match(source, /grid-template-columns:repeat\(auto-fit,minmax\(min\(100%,440px\),1fr\)\)/);
  assert.match(source, /GUILD_TOKEN_CREDIT_CONVERSIONS\.slice\(0, 4\)/);
  assert.match(source, /GUILD_TOKEN_CREDIT_CONVERSIONS\.slice\(4\)/);
  ...
  assert.equal((source.match(/repeat\(auto-fit,minmax\(min\(100%,300px\),1fr\)\)/g) || []).length, 2);
