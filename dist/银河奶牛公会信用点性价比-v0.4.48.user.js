...
window.MwiGuildCreditVersion = "0.4.48";
  ...
  function renderMaterialTotals(results, totals, estimate, hasInventory, creditMaterialPlans, materialInventory) {
    ...
    const materials = totals.sort(materialOrder).map((item) => {
      ...
      const conversionMarkup = row && row.missing > 0 && isGuildCredit
        ? plan
          ? `${iconMarkup(plan.itemHrid, itemNameForMaterial(plan.itemHrid))}<span>${escapeHtml(itemNameForMaterial(plan.itemHrid))}库存 ${hasInventory ? formatNumber(Number(materialInventory && materialInventory[plan.itemHrid]) || 0) : "未读取"} · 需 ${formatNumber(plan.requiredItems)}（${formatNumber(plan.itemCount)} 个 -> ${formatNumber(plan.creditCount)} 点）</span>`
          : "最优兑换：暂无可用市场价格"
        : "";
      return `<div class="mwi-material-row">${iconMarkup(item.itemHrid, itemNameForMaterial(item.itemHrid))}<span class="mwi-material-copy"><span class="mwi-material-name">${escapeHtml(itemNameForMaterial(item.itemHrid))}</span><small>${hasInventory ? inventoryText : "库存未读取"}</small>${conversionMarkup ? `<small class="mwi-material-plan">${conversionMarkup}</small>` : ""}</span><strong>${formatNumber(item.count)}</strong></div>`;
    }).join("");
