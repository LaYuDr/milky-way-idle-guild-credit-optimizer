...
window.MwiGuildCreditVersion = "0.4.49";
...
(function () {
  ...
  function renderUpgradeCostSummary(estimate, hasInventory) {
    ...
    return `<section class="mwi-upgrade-cost-summary"><div class="mwi-upgrade-cost-title">成本概览</div><div><span>${totalLabel}</span><strong>${renderUpgradeCostText(estimate.totalGold, estimate.guildTokensRequired)}</strong></div><div><span>${missingLabel}</span><strong>${renderUpgradeCostText(estimate.missingGold, estimate.guildTokensMissing)}</strong></div>${inventoryNote}${priceNote}</section>`;
  }
