...
- 神龛和起始／目标等级改用主题一致的自定义选择器，支持分组、效果说明、禁用原因与键盘操作；同一神龛的每级效果合并为一条说明；已选神龛计划支持独立折叠并保存状态，收起后保留名称、分类与等级范围，不影响材料汇总。
...
- Trial history: sum valid combat metric multiples; add optional combat share and average columns; remove per-record raw disclosures and update labels.
...
- 设置中的神龛选择新增游戏原图及每级增益说明，兼容首级与后续增量差异并保留异步图片加载后的键盘焦点。目标信用点默认值改为 100，保留已保存的合法输入。试炼个人总览将生活与战斗标题置于表格上方，新增总相对人均列和全部项目合计行，按有效样本汇总并改善中英文窄栏换行。
...
- 玩家排行榜将计入周数改为样本数：生活、战斗按每周实际参试分别最多计 1 个样本，未参加不计样本，合并榜为两类之和；实际参试的零贡献和未知数值记录仍计样本。样本计数与平均值分母分离，保留确认缺席计零及原有排名计算，补充单元测试和中英文浏览器验证。
