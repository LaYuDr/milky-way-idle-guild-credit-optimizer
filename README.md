...
- 市场价格以游戏公开的 `game_data/marketplace.json` 快照为基线；玩家主动打开某个商品的原生市场页面后，插件会被动保存游戏已返回的该商品最新买卖价并优先用于估算。该缓存会跨页面刷新与浏览器重启保留，直到同商品的新订单簿替换它，或时间戳真正前进的公开快照覆盖它。插件自身不发送交易或订单簿请求。
